...
import { MdDelete } from "react-icons/md"; // ← иконка удаления
import {
...
const Review = ({ productId }: ReviewProps) => {
	...
	const handleSubmit = async () => {
		if (rating === 0) {
			toast.warning("Пожалуйста, выберите рейтинг.", {
				position: "top-center",
			});
			return;
		}
		...
		try {
			...
			if (error.response?.data?.detail === "Review already exists") {
				toast.warning("Вы уже оставляли отзыв на этот товар.", {
					position: "top-center",
				});
			} else if (error.response?.status === 401) {
				toast.warning("Авторизуйтесь, чтобы оставить отзыв.", {
					position: "top-center",
				});
			} else {
									...
									<div className="flex items-center gap-2">
										...
										{isMyReview && (
											<button
												...
												className="text-red-500 hover:text-red-700 transition-colors px-3"
