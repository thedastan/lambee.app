		...
		<section>
			...
			<div className="md:p-4 p-0 mt-4 md:mt-0">
				{data?.data.detail.map((el, index) => (
					<div
						key={index}
						className="bg-white p-8 border-b border-[#E4E4E7] flex justify-between items-center gap-2">
						<div className="flex gap-2">
							<Image src={logo} alt="logo" />
							<div className="w-full max-w-[220px] flex flex-col gap-2">
								<Title>{el.title}</Title>
								<Description className="text-[#161616be]">
									{el.description}
								</Description>
						...
						<Description>{formatDate(el.created_at)}</Description>
					</div>
				))}
			</div>
		</section>
